...
const initDb = () => {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS projects (
      ...
      is_routine BOOLEAN NOT NULL DEFAULT 1,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    ...
    CREATE TABLE IF NOT EXISTS checklists (
      ...
      project_id INTEGER,
      title TEXT NOT NULL,
      active_days TEXT NOT NULL DEFAULT '0,1,2,3,4,5,6', -- Days of week, 0=Sun, 6=Sat
  ...
  const tableInfoProjects = db.pragma('table_info(projects)');
  const hasProjectActiveDays = tableInfoProjects.some(col => col.name === 'active_days');
  if (!hasProjectActiveDays) {
    db.exec("ALTER TABLE projects ADD COLUMN active_days TEXT NOT NULL DEFAULT '0,1,2,3,4,5,6'");
  }

  const hasProjectIsRoutine = tableInfoProjects.some(col => col.name === 'is_routine');
  if (!hasProjectIsRoutine) {
    db.exec("ALTER TABLE projects ADD COLUMN is_routine BOOLEAN NOT NULL DEFAULT 1");
  }

  // Migration: Add color to projects if missing
  const hasProjectColor = tableInfoProjects.some(col => col.name === 'color');
  if (!hasProjectColor) {
    db.exec("ALTER TABLE projects ADD COLUMN color TEXT DEFAULT '#6366f1'");
  }

  // Migration: Add parent_id to projects if missing (sub-projects)
  const hasProjectParentId = tableInfoProjects.some(col => col.name === 'parent_id');
  if (!hasProjectParentId) {
    db.exec('ALTER TABLE projects ADD COLUMN parent_id INTEGER REFERENCES projects(id) ON DELETE SET NULL');
  }

  // Migration: Add order_index to projects if missing
  const hasProjectOrderIndex = tableInfoProjects.some(col => col.name === 'order_index');
  if (!hasProjectOrderIndex) {
    db.exec('ALTER TABLE projects ADD COLUMN order_index INTEGER DEFAULT 0');
  }

  // Data Migration: Ensure existing checklists are assigned to a project
  const checklistsWithoutProject = db.prepare('SELECT id, user_id FROM checklists WHERE project_id IS NULL').all();
  if (checklistsWithoutProject.length > 0) {
    db.transaction(() => {
      const usersWithOrphanChecklists = [...new Set(checklistsWithoutProject.map(c => c.user_id))];
      usersWithOrphanChecklists.forEach(userId => {
        // Create a default project for the user
        const insertProject = db.prepare('INSERT INTO projects (user_id, title) VALUES (?, ?)');
        const result = insertProject.run(userId, 'הפרויקט הראשון שלי');
        const newProjectId = result.lastInsertRowid;

        // Assign orphan checklists to this project
        const updateChecklists = db.prepare('UPDATE checklists SET project_id = ? WHERE user_id = ? AND project_id IS NULL');
        updateChecklists.run(newProjectId, userId);
      });
    })();
  }
