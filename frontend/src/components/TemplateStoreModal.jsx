    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const pRes = await fetch(`${apiUrl}/users/${userId}/projects`, {
                ...
                body: JSON.stringify({ title: title.trim(), is_routine: projectType === 'routine' })
            });
            ...
            if (pRes.ok) {
                ...
                // Pass a flag to the parent indicating this was a magical template creation
                        ...
                        fetching ? (
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '1.25rem', alignContent: 'flex-start' }}>
                                {templates.map((tmp, idx) => (
                                    <div key={tmp.id} className={`card hover-scale stagger-${(idx % 4) + 1}`} onClick={() => { setSelectedTemplate(tmp); setTitle(tmp.title); setProjectType('routine'); }} style={{
                                        ...
                                        <div style={{ marginTop: 'auto', background: 'var(--hover-bg)', borderRadius: '6px', padding: '0.6rem', border: '1px solid rgba(0,0,0,0.03)' }}>
                                            <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                                                {tmp.items && tmp.items.slice(0, 2).map((item, i) => (
                                                    <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.8rem', color: 'var(--text-primary)' }}>
                                                        <div style={{ width: '4px', height: '4px', borderRadius: '50%', background: 'var(--primary-color)', flexShrink: 0 }}></div>
                                                        <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.content}</span>
                                                    </li>
                                                ))}
                                                {tmp.items && tmp.items.length > 2 && (
                                                    <li style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.1rem', paddingRight: '0.6rem' }}>
                                                        + ועוד {tmp.items.length - 2} מחכות בפנים
                                                    </li>
                                                )}
                                                {(!tmp.items || tmp.items.length === 0) && (
                                                    <li style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontStyle: 'italic' }}>אין משימות בתבנית זו</li>
                                                )}
                                            </ul>
                                        </div>
                                    </div>
                            ...
                            <div className="form-group">
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                                    <button
                                        ...
                                        style={{
                                            border: `1px solid ${projectType === 'routine' ? 'var(--primary-color)' : '#ddd'}`,
                                            borderRadius: '6px', padding: '0.75rem', cursor: 'pointer',
                                            background: projectType === 'routine' ? 'rgba(16, 185, 129, 0.05)' : 'var(--bg-color)',
                                            display: 'flex', alignItems: 'center', gap: '0.5rem', justifyContent: 'center',
                                            transition: '0.2s', color: projectType === 'routine' ? 'var(--primary-color)' : 'var(--text-primary)'
                                        }}
                                    >
                                        <Repeat size={18} />
                                        <span style={{ fontSize: '0.9rem', fontWeight: projectType === 'routine' ? 600 : 400 }}>שגרה קבועה</span>
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => setProjectType('once')}
                                        style={{
                                            border: `1px solid ${projectType === 'once' ? 'var(--primary-color)' : '#ddd'}`,
                                            borderRadius: '6px', padding: '0.75rem', cursor: 'pointer',
                                            background: projectType === 'once' ? 'rgba(16, 185, 129, 0.05)' : 'var(--bg-color)',
                                            display: 'flex', alignItems: 'center', gap: '0.5rem', justifyContent: 'center',
                                            transition: '0.2s', color: projectType === 'once' ? 'var(--primary-color)' : 'var(--text-primary)'
                                        }}
                                    >
                                        <Target size={18} />
                                        <span style={{ fontSize: '0.9rem', fontWeight: projectType === 'once' ? 600 : 400 }}>יעד חד-פעמי</span>
                                    </button>
                                </div>
                            </div>
                        </form>
