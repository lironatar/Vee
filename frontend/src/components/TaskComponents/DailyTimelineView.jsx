    ...
    return (
        <div
            ...
            style={{
                ...
                alignItems: 'center',
                justifyContent: 'center',
                boxShadow: isDragging ? '0 12px 24px rgba(var(--primary-rgb), 0.4)' : '0 4px 12px rgba(0,0,0,0.15)',
...
const DailyTimelineView = ({
    ...
    const handleDragEnd = (event) => {
        ...
        if (active.id === 'draggable-fab' && over && over.data.current?.type === 'TimelineSlot') {
            const { dateStr, time } = over.data.current;
            if (onAddTaskClick) onAddTaskClick(dateStr, time);
        }
    };

    // Current time indicator position
    const [nowPercent, setNowPercent] = React.useState(0);
    useEffect(() => {
        const updateNow = () => {
            const now = new Date();
