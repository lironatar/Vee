        ...
        <div ref={setNodeRef} style={style}>
            <div {...attributes} {...listeners} style={{ display: 'flex', alignItems: 'center', width: '100%', cursor: 'default' }}>
                <Link
                    ...
                    style={{ flexGrow: 1 }}
                >
                    <Hash size={16} color={projColor} strokeWidth={2} style={{ opacity: 0.6 }} />
            ...
            {children.length > 0 && !isDragging && (
                <div style={{
                    ...
                    {children.map(child => {
                        ...
                        return (
                            <Link key={child.id} to={`/project/${child.id}`} onClick={() => { if (window.innerWidth <= 992) onToggle(); }} className={`nav-link project-link sidebar-menu-item ${isChildActive ? 'active' : ''}`} style={{ padding: '0.35rem 0.5rem', fontSize: '0.9rem' }}>
                                <Hash size={14} color={childColor} strokeWidth={2} style={{ opacity: 0.6 }} />
